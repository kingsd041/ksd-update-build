...
#DOWNLOAD_RANCHER_VERSION=2.7

# 下载 rancher 2.7  的 rancher-image.txt
export RANCHER_VERSION=`curl -L -s https://api.github.com/repos/rancher/rancher/git/refs/tags | jq -r .[].ref | awk -F/ '{print $3}' | grep v | awk -Fv '{print $2}' | grep -v [a-z] | awk -F"." '{arr[$1"."$2]=$3}END{for(var in arr){if(arr[var]==""){print var}else{print var"."arr[var]}}}' | sort -u -t "." -k1nr,1 -k2nr,2 -k3nr,3 | grep -v ^0. | grep -v ^1. | grep -E '^2.7'`
...
cd rancher 
git checkout -b  local/v2.7 remotes/origin/release/v2.7
